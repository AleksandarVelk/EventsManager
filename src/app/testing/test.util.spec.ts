...
import { ClassProvider, NgModule, Provider, Type } from '@angular/core';
import { ComponentFixture, getTestBed, MetadataOverride, TestBed, TestModuleMetadata } from '@angular/core/testing';

export class TestUtil {
  private static originalTimeoutInterval: number;

  public static beforeEachCompiler(component: any, config: TestModuleMetadata, override?: IOverride): Promise<{ fixture: ComponentFixture<any>, instance: any, injector: TestBed }> {
  ...
  public static mergeProviders(target: Provider[], source: Provider[]): Provider[] {
    source.forEach(cProvider => {
      if (cProvider instanceof Object) {
        let index = target.findIndex((mProvider: any) => mProvider && mProvider.provide ? mProvider.provide === (<ClassProvider>cProvider).provide : (<Function>mProvider).name === (<Function>cProvider).name);
        if (index === -1) {
